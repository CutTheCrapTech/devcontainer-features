...
class TestSecretManagerBaseConfigFile:
  ...
  def test_find_config_file_home_config(self, tmp_path: Any) -> None:
    ...
    with (
      ...
      patch("auto_secrets.secret_managers.base.Path.home", return_value=tmp_path),
    ):
      result = manager._find_config_file()
      assert result == config_file
  ...
  def test_load_config_file_invalid_json(self, tmp_path: Any) -> None:
    ...
    with (
      patch.object(manager, "_find_config_file", return_value=config_file),
      pytest.raises(ConfigurationError, match="Invalid JSON in config file"),
    ):
  ...
  def test_load_config_file_not_json_object(self, tmp_path: Any) -> None:
    ...
    with (
      patch.object(manager, "_find_config_file", return_value=config_file),
      pytest.raises(ConfigurationError, match="Config file must contain a JSON object"),
    ):
      manager._load_config_file()
  ...
  def test_load_config_file_read_error(self, tmp_path: Any) -> None:
    ...
    with (
      ...
      pytest.raises(ConfigurationError, match="Failed to read config file"),
    ):
      manager._load_config_file()
  ...
  def test_get_secret_value_not_found_required(self) -> None:
    ...
    with (
      patch.object(manager, "_load_config_file", return_value={}),
      pytest.raises(ConfigurationError, match="Required secret 'REQUIRED_SECRET' not found"),
    ):
  ...
  def test_get_secret_value_config_file_debug_logging(self, tmp_path: Any) -> None:
    ...
    with (
      patch.object(manager, "_find_config_file", return_value=config_file),
      patch.object(manager, "log_debug") as mock_log,
    ):
