...
_setup_environment_change_detection() {
    export LAST_KNOWN_ENVIRONMENT=$(get_current_environment)
...
test_environment_mapping() {
    ...
    configured_branches=($(echo "$DEV_ENV_MANAGER_BRANCH_MAPPING_JSON" | jq -r 'keys[]'))
    ...
    local all_branches=()
    for branch in "${configured_branches[@]}" "${test_branches[@]}"; do
