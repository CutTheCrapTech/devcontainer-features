...
# Azure configuration
readonly AZURE_CLI_MIN_VERSION="2.0.0"
...
fetch_azure_secrets() {
    ...
    # Fetch secret value
    local secret_string
    secret_string=$(_get_azure_secret_value "$secret_name")
    ...
    parsed_secrets=$(_parse_azure_secret_string "$secret_string")
...
test_azure_connection() {
    ...
    local account_info
    account_info=$(_get_azure_account_info 2>/dev/null)
    if [[ $? -eq 0 ]]; then
