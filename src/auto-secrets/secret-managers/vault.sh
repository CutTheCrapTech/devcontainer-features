...
_get_kv_version() {
  ...
  local response
  if response=$(_vault_api_request "GET" "sys/mounts"); then
    local version
...
_fetch_vault_kv1_secret() {
  ...
  local response
  if response=$(_vault_api_request "GET" "$mount/$path"); then
...
_fetch_vault_kv2_secret() {
  ...
  local response
  if response=$(_vault_api_request "GET" "$mount/data/$path"); then
...
test_vault_connection() {
  ...
  local response
  if response=$(_vault_api_request "GET" "sys/mounts" 2>/dev/null); then
    local mount_exists
...
authenticate_vault() {
  # This function is not implemented in this script yet, but used by manager-interface.sh
  log_warn "Vault authentication function not implemented"
}

# Set up Vault as the active secret manager interface
export -f fetch_vault_secrets authenticate_vault test_vault_connection
