...
fetch_aws_secrets() {
  ...
  # Fetch secret value
  local secret_string
  if secret_string=$(_get_aws_secret_value "$secret_name"); then
  ...
  if parsed_secrets=$(_parse_secret_string "$secret_string"); then
    local secret_count
...
test_aws_connection() {
  ...
  local identity
  if identity=$(_get_aws_identity 2>/dev/null); then
