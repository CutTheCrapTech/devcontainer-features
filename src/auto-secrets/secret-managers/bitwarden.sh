...
readonly BW_CLI_MIN_VERSION="2.0.0"
readonly BW_SESSION_TIMEOUT="1h"
...
_authenticate_bitwarden_session() {
    ...
    if [[ -n "$client_id" ]] && [[ -n "$client_secret" ]]; then
        ...
        local session_token
        session_token=$(bw login --apikey --raw 2>/dev/null)

        if [[ $? -eq 0 ]] && [[ -n "$session_token" ]]; then
    ...
    if [[ -n "$email" ]] && [[ -n "$password" ]]; then
        ...
        local session_token
        session_token=$(echo "$password" | bw login "$email" --raw 2>/dev/null)
    ...
    if [[ "$status" == "locked" ]]; then
        ...
        if [[ -n "$password" ]]; then
            local session_token
            session_token=$(echo "$password" | bw unlock --raw 2>/dev/null)
...
fetch_bitwarden_secrets() {
    ...
    # Fetch item
    local item_json
    item_json=$(_get_bitwarden_item "$item_name")
    ...
    parsed_secrets=$(_extract_secrets_from_item "$item_json")
...
export -f fetch_bitwarden_secrets test_bitwarden_connection
