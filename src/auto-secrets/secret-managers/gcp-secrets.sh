...
fetch_gcp_secrets() {
  ...
  # Fetch secret value
  local secret_string
  if secret_string=$(_get_gcp_secret_value "$secret_name"); then
  ...
  if parsed_secrets=$(_parse_gcp_secret_string "$secret_string"); then
    local secret_count
...
test_gcp_connection() {
  ...
  local account_info
  if account_info=$(_get_gcp_account_info 2>/dev/null); then
...
export -f fetch_gcp_secrets authenticate_gcp test_gcp_connection
