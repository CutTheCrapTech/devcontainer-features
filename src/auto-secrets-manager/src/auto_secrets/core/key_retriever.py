...
class KeyRetriever:
    ...
    def derive_smk_from_ssh_agent(self) -> int:
        ...
        if not target_key:
            raise RuntimeError(
                f"Could not find key with comment '{self.ssh_agent_key_comment}' in SSH agent."
            )
    ...
    def prompt_ssh_agent_biometric(self) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to prompt SSH agent for biometric authentication: {e}"
            )
