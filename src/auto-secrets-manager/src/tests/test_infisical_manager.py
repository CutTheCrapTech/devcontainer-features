...
class TestInfisicalSecretManager:
  ...
  def test_get_client_initialization(self, mock_sdk_client: MagicMock) -> None:
    ...
    with patch.object(manager, "_authenticate"):
      ...
      mock_sdk_client.assert_called_once_with(host="https://app.infisical.com", cache_ttl=300)
  ...
  def test_authenticate_success(self, mock_sdk_client: MagicMock) -> None:
    ...
    mock_universal_auth.login.assert_called_once_with(client_id="test_client_456", client_secret="test_secret")
  ...
  def test_fetch_secrets_api_error(self, mock_sdk_client: MagicMock) -> None:
    ...
    with (
      patch.object(manager, "_get_client", return_value=mock_client_instance),
      pytest.raises(SecretManagerError, match="Failed to fetch secrets"),
    ):
...
class TestInfisicalSecretManagerIntegration:
  ...
  def test_full_workflow(self, mock_sdk_client: MagicMock) -> None:
    ...
    mock_universal_auth.login.assert_called_with(client_id="integration_client", client_secret="integration_secret")
